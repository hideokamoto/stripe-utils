import type { DeclineCodeInfo, DeclineCodeResponse, DeclineCodes } from '../../types'
...
const declineCodes: DeclineCodes = {
	...
	fraudulent: {
		...
		nextSteps:
			'Do not report more detailed information to your customer. Instead, present as you would the generic_declinedescribed below.',
		nextUserAction: 'Please contact your card issuer for more information.',
		translations: {
			ja_JP: {
	...
	merchant_blacklist: {
		...
		nextSteps:
			'Do not report more detailed information to your customer. Instead, present as you would the generic_declinedescribed above.',
		nextUserAction: 'Please contact your card issuer for more information.',
		translations: {
			ja_JP: {
				description: 'カードは未知の理由で拒否されました。',
...
const getDeclineCode = (declineCode = ''): DeclineCodeInfo | Record<string, never> => {
	if (!declineCode) return {}
	const code = declineCodes[declineCode]
	return code || {}
}

export const getDeclineDescription = (declineCode = ''): DeclineCodeResponse => {
